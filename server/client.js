...
const prefixes={'2° ANO TÉCNICO MATUTINO':'MAT','2° ANO TÉCNICO VESPERTINO':'VESP','2° ANO TÉCNICO VESPERTINO NAZIR':'NAZIR','2° ANO TÉCNICO TRINDADE':'TRI2','1° ANO TÉCNICO TRINDADE':'TRI1'};
...
function saveDraft(){try{localStorage.setItem('bd-draft-v2',JSON.stringify(draft));return true;}catch{$('submitErr').textContent='O navegador não permite salvar uma cópia local. Mantenha esta página aberta até confirmar o envio.';$('submitErr').classList.remove('hidden');return false;}}
...
async function startExam(){
 ...
 try{const data=await request('/api/start',b);draft={...b,id:crypto.randomUUID(),questions:data.questions,answers:Array(40).fill(null),saidas:0,forced:false,pending:false};saveDraft();showExam();}catch(e){showError('startErr',e.message);}finally{btn.disabled=false;btn.textContent='Começar prova';}
}
...
async function submitExam(e,force=false){
 ...
 try{const data=await request('/api/submit',{id:draft.id,name:draft.name,turma:draft.turma,codigo:draft.codigo,answers:draft.answers,saidas:draft.saidas,forced:draft.forced});
  ...
  try{localStorage.removeItem('bd-draft-v2');}catch{}draft=null;
...
async function resetCode(codigo){if(!confirm('Liberar '+codigo+' para outra tentativa? A nota anterior será preservada.'))return;try{await request('/api/teacher/reset',{codigo},true);await refreshResults();}catch(e){alert(e.message);}}
